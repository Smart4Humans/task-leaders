...
**Phase 0 — admin_alert / observability for relay & notification failures, plus `last_activity_at` hygiene** (no template / queue work; visibility-only):
- **WC-2 first-time web-client session-window risk** — Marketplace ACCEPT sends client WC-2 via plain `sendWhatsApp()`, not `sendTemplateWhatsApp()`. If a web-submitted client has no open 24-hour WhatsApp session, Twilio rejects the send. Phase 0 fix: surface the failure as an `admin_alert` instead of letting it silently disappear. Phase 1 fix (separately): wire a Meta-approved Content SID for WC-2 and route via `sendTemplateWhatsApp()`.
- **Marketplace ACCEPT/DECLINE notification observability gap** — some sends use raw `sendWhatsApp()` / `logMessage()` and do not create `admin_alerts` rows on failure, so silent send failures are invisible to ops. Phase 0 fix: pair every send with an `admin_alert` insert on failure.
- **Reject-branch outbound observability gap** — the prior reject-branch "This request is no longer available" outbound was not logged to `message_log`. Same Phase 0 family as `escalateAmbiguousReply` (§11 above).
- **Recipient `last_activity_at` hygiene in Marketplace ACCEPT** — the client (recipient) session may be stamped `last_activity_at` even when the client did not initiate the inbound, polluting the 24-hour-window heuristic. Phase 0 fix: only advance `last_activity_at` on actual inbound from that participant.

**Phase 1 — queued-message + reactivation-template workflow for closed-session recipients** (template work + new queue table):
