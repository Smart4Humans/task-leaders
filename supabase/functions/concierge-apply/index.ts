...
// Body: { name (or full_name), email, whatsapp, company, role }
...
Deno.serve(async (req) => {
  ...
  // Accept both `name` and `full_name` for compatibility with the concierge form
  const name    = clean(body.name || body.full_name);
  const email   = clean(body.email).toLowerCase();
  const whatsapp = clean(body.whatsapp);
  const company = clean(body.company);
  const role    = clean(body.role);

  if (!name)                        return error("validation_error", "Name is required");
  ...
  const { data, error: insErr } = await supabase
    ...
    .insert({ name, email, whatsapp, company: company || null, role: role || null })
    .select("id")
    .single();
